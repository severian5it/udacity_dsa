...
# my solution, similar to james problem! but they must be absolutely sorted!! Otherwise you need O(N^2)
def min_platforms(arrival, departure):
    ...
    i, j, platform, max_required = 0, 0, 0, 0
    while i < len(arrival) and j < len(departure):
        if arrival[i] < departure[j]:
            ...
            platform -=1
            j +=1
        max_required = max(max_required, platform)
