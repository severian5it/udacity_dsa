'''
Heaps are CBT --> Complete Balanced Tree, balanced because left and right have same height.
Complete is that only the last element are not complete.
...
get minimum is O(1), insert and remove are at worst O(h) where h is the tree height, which is more or less O(logN)

This is best performing why to manage a priority Queue, otherwise
Array --> worst case O(N)
Linked List --> worst case O(N)
Hashmap  --> worst case O(N)
Binary Tree -> O(h) where h is the height of tree which is O(logN)
