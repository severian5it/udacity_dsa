...
# here is used bucket_array and modulo to limit number of buckets
class HashMap:
...
# Separate Chaining
'''
Separate chaining is a clever technique where we use the same bucket to store multiple objects. 
...
class HashMap:
    ...
    def put(self, key, value):  # The key is a string, and value is numeric
        ...
        new_node.next = head
        self.bucket_array[
            bucket_index] = new_node  # Prepend the new node in the beginning of the linked list
        self.num_entries += 1
...
# rehashing, this happens when we need to increase the number of buckets beyond initial size
